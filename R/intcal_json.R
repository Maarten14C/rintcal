...
intcal.extract.record <- function(i) {
  if(!"intcal" %in% ls(.GlobalEnv)) # then load the intcal json file
...
#' @return A plot of the calendar and radiocarbon ages, indicating uncertainties (error bars) and age blocks where relevant (e.g., for trees where blocks of >1 rings were dated).
#' @param i The IntCal record, in the order as they appear in the IntCal20 json file (140 entries). Must be a single integer between 1 and 140. 
...
#' @param as.Delta Return the Delta14C values, calculated from the C14 ages (columns 2 and 3). Defaults to \code{as.Delta=FALSE}.
...
intcal.plot.record <- function(i, col=rgb(0, 0, 1, .5), pch=19, pch.cex=.3, lwd=1, lty=1, cal.lim=c(), C14.lim=c(), add=FALSE, cal.lab=c(), C14.lab=c(), ka=FALSE, as.F=FALSE, as.pMC=FALSE, as.Delta=FALSE, draw.z=TRUE, draw.calsigma=TRUE, grid=FALSE, grid.lty=2, grid.col=rgb(0,0,0,.5), draw.cc=1, cc.col=rgb(0,.5,0, .5), legend.loc="topleft", legend.cex=.5, mgp=c(2.5, 0.8, 0)) {
  ...
  if(as.F || as.pMC || as.Delta) {
    translated <- C14.F14C(dat$radiocarbon$r_date, dat$radiocarbon$r_date_sigma)
    if(as.pMC) {
      translated <- 100 * translated
    }
    if(as.Delta) { # then adapt cc and record's C14 ages
      cc <- cbind(cc[,1], F14C.er.D14C(cc[,2], cc[,3], cc[,1]))
      translated <- F14C.er.D14C(translated[,1], translated[,2], dat$radiocarbon$calage)
    }
    ...
    if(length(cal.lab) == 0)
      if(ka)
        cal.lab <- "kcal BP" else
          cal.lab <- "cal BP"
    if(length(C14.lab) == 0)
      if(as.pMC) C14.lab <- "pMC" else
        if(as.Delta) C14.lab <- expression(Delta^{14} * "C ("~"\u2030)") else
          if(as.F) C14.lab <- "F14C" else
            if(ka)
...
#' @description Write the intcal.json file that comes with the rintcal package to somewhere local. This can be useful if you want to avoid repeatedly downloading the json file from intchron.org.
