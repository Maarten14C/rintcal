...
# todo: migrate caldist and hpd from clam?
...
#' @name draw.calibrationcurve
#' @title Draw an IntCal calibration curve.
...
#' @export
draw.calibrationcurve <- function(cal1=-50, cal2=55e3, cc1="IntCal20", cc2=NA, BCAD=FALSE, cal.lab=NA, cal.rev=FALSE, c14.lab=NA, c14.lim=NA, c14.rev=FALSE, ka=FALSE, cc1.col=rgb(0,0,1,.5), cc1.fill=rgb(0,0,1,.2), cc2.col=rgb(0,.5,0,.5), cc2.fill=rgb(0,.5,0,.2), add=FALSE, bty="l", ...) {

  # read and narrow down the calibration curve(s)
  cc.1 <- copyCalibrationCurve(cc1)
  if(BCAD)
    cc.1[,1] <- 1950 - cc.1[,1] 
...
intcal.data <- function(cal1, cal2, cc1="IntCal20", cc2=NA, calcurve.data="IntCal20", BCAD=FALSE, cal.lab=NA, cal.rev=FALSE, c14.lab=NA, c14.lim=NA, c14.rev=FALSE, ka=FALSE, cc1.col=rgb(0,0,1,.5), cc1.fill=rgb(0,0,1,.2), cc2.col=rgb(0,.5,0,.5), cc2.fill=rgb(0,.5,0,.2), data.cols=1:8, data.pch=c(1,2,5,6,15:19), pch.cex=.5, legend.loc="topleft", legend.ncol=2, legend.cex=0.7, cc.legend="bottomright", bty="l",  ...) {
  ...
  if(tolower(calcurve.data) == "shcal20") {
    dat <- system.file("extdata/", "shcal20_data.txt", package = "IntCal")
    sourcesdat <- system.file("extdata/", "shcal20_data_sources.txt", package = "IntCal")
    sourcesdat <- read.table(sourcesdat, nrows=9, sep=",")
  }
  dat <- read.table(dat, header=TRUE, sep=" ")
  dat <<- dat
  sourcesdat <<- sourcesdat
